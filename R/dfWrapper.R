...
mappingObjDF <- function(df,
  ...
  if(!is.na(spot.ID)){
    ...
    probes = try(as.character(df[,spot.ID]))
  ...
  if(!is.na(loc)){
    ...
    startData = try(df[,loc.start])
    endData = try(df[,loc.stop])
  ...
  if(!is.na(additional[1])){
    ...
    # columns have been specified by name or numeric indications
      ...
      if(class(additional) == "character"){
        ...
        if(length(mm) != 0){
          extraInc = intersect(1:dim(df)[2], mm)
          if(length(extraInc) > 0)mapping.info = cbind(mapping.info, df[,extraInc])
          mm = extraInc
        }
      }
  ...
  # if additional is left as NA
    ...
    cdx = setdiff(1:dim(df)[2], subC)
    adnam = names(mapping.info)

    mapping.info = cbind(mapping.info, df[,cdx])
  ...
  if(!is.null(dim(links))){
    ...
    if(!is.na(links[1])){
      ...
      if(class(links) == "character"){
        ...
        if(length(which(is.na(mm))) > 0) mm = mm[-which(is.na(mm))]
        if(length(mm) != 0){
          linksInc = intersect(1:dim(df)[2], mm)
          mm = linksInc
  ...
  if(!is.null(dim(images))){
    ...
    if(!is.na(images[1])){
      ...
      if(class(images) == "character"){
        ...
        if(length(which(is.na(mm))) > 0) mm = mm[-which(is.na(mm))]
        if(length(mm) != 0){
          imagesInc = intersect(1:dim(df)[2], mm)
          mm = imagesInc
